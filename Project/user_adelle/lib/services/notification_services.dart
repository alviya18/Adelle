...
class RoutineNotificationService {
  ...
  static const int PERIOD_REMINDER_ID = 1000;
  static const int LATE_PERIOD_REMINDER_ID = 1001;
  ...
  static Future<void> scheduleOneTimeNotification({
    ...
    try {
      const AndroidNotificationDetails androidDetails = AndroidNotificationDetails(
        ...
        channelDescription: 'Reminders for Data Entering',
  ...
  static Future<void> schedulePeriodReminder(DateTime predictedDate) async {
    try {
      // Calculate the reminder date (3 days before)
      ...
      // Convert to TZDateTime and set time to 9:00 AM
      ...
      // Schedule the pre-period reminder
      await _notificationsPlugin.zonedSchedule(
        PERIOD_REMINDER_ID,
        'Period Reminder',
      ...
      // Schedule the late period reminder (1 day after predicted date)
      // Also set to 9:00 AM
      final lateReminderDate = tz.TZDateTime(
        tz.local,
        predictedDate.year,
        predictedDate.month,
        predictedDate.day + 1, // 1 day after
        7, // 9 AM
        0,  // 0 minutes
      );

      await _notificationsPlugin.zonedSchedule(
        LATE_PERIOD_REMINDER_ID,
        'Period Tracking Reminder',
        'Your period was expected yesterday. Don\'t forget to mark it if it has started!',
        lateReminderDate,
        notificationDetails,
        androidScheduleMode: canUseExact
            ? AndroidScheduleMode.exactAllowWhileIdle
            : AndroidScheduleMode.inexactAllowWhileIdle,
        payload: 'late_period_reminder',
      );

      print('Period reminders scheduled for: $scheduledDate and $lateReminderDate');
      
      // Check pending notifications to confirm scheduling
      await checkPendingNotifications();
    } catch (e) {
      print('Error scheduling period reminders: $e');
    }
  }

  // Add method to cancel all period-related notifications
  static Future<void> cancelPeriodReminders() async {
    await _notificationsPlugin.cancel(PERIOD_REMINDER_ID);
    await _notificationsPlugin.cancel(LATE_PERIOD_REMINDER_ID);
    print('All period reminders canceled');
  }
}
